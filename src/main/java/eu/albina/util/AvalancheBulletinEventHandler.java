...
public class AvalancheBulletinEventHandler implements IEventHandler {
	...
	public final PdfFont openSansRegularFont;
	public final PdfFont openSansBoldFont;
	public final PdfFont openSansLightFont;

	public AvalancheBulletinEventHandler(LanguageCode lang, List<AvalancheBulletin> bulletins, boolean grayscale,
			PdfFont openSansRegularFont, PdfFont openSansBoldFont, PdfFont openSansLightFont) {
		this.lang = lang;
	...
	public void handleEvent(Event event) {
		...
		if (!publicationDate.isEmpty())
			pdfCanvas.beginText().setFontAndSize(openSansRegularFont, 8).moveText(20, pageSize.getTop() - 75)
					.setColor(greyDarkColor, true).showText(GlobalVariables.getPublishedText(lang) + publicationDate)
					.endText();
		...
		pdfCanvas.beginText().setFontAndSize(openSansBoldFont, 8)
				.moveText(buttonRectangle.getLeft() + 15, buttonRectangle.getBottom() + 9).setColor(whiteColor, true)
				.showText(urlString).endText();
		...
		pdfCanvas.beginText().setFontAndSize(openSansRegularFont, 12).moveText(pageSize.getWidth() / 2 - width / 2, 20)
				.setColor(greyDarkColor, true).showText(pageText).endText();
